...
```{r fig.height = 4, fig.width = 4, fig.cap= "2018 Ozone Levels", fig.align='left',echo=FALSE, message=FALSE, warning=FALSE}
# Filter for 2018 data
Ozone_2018 <- epa_data %>%
  filter(Year==2018)
# Create a boxplot
Figure_1 <- ggplot(Ozone_2018, aes(x=Month, y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Ozone levels (ppb)") +
  scale_x_continuous(breaks = 1:12, labels = month.abb, limits = c(1,12))
# Print figure
Figure_1
```

```{r fig.height = 4, fig.width = 4, fig.cap="2019 Ozone Levels", fig.align='left', echo=FALSE, message=FALSE, warning=FALSE}
# Filter for 2019 data
Ozone_2019 <- epa_data %>%
  filter(Year==2019)
# Create a boxplot
Figure_2 <- ggplot(Ozone_2019, aes(x=Month, y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Ozone levels (ppb)") +
  scale_x_continuous(breaks = 1:12, labels = month.abb, limits = c(1,12))
# Print figure
