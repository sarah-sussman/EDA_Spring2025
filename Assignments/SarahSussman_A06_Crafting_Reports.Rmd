...
```{r fig.height = 4, fig.width = 6, fig.cap= "2018 Monthly Ozone Levels", fig.align='left',echo=FALSE, message=FALSE, warning=FALSE}
# Filter for 2018 data and group by month
Ozone_2018 <- epa_data %>%
  filter(Year==2018)

# Create a boxplot
Figure_1 <- ggplot(Ozone_2018, aes(x=factor(Month), y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Concentration (µg/m³)", title = "Monthly Ozone Concentrations (2018)") +
...
```{r fig.height = 4, fig.width = 6, fig.cap="2019 Monthly Ozone Levels", fig.align='left', echo=FALSE, message=FALSE, warning=FALSE}
# Filter for 2019 data
Ozone_2019 <- epa_data %>%
  filter(Year==2019)

# Create a boxplot
Figure_2 <- ggplot(Ozone_2019, aes(x=factor(Month), y=Ozone)) +
  geom_boxplot() +
  labs(x="Month", y="Concentration (µg/m³)", title = "Monthly Ozone Concentrations (2019)") +
  scale_x_discrete(breaks = 1:12, labels = month.abb)
# Print figure
Figure_2
```
...
## Git Repository
[Sarah Sussman's Git Repository for EDA Spring 2025](https://github.com/sarah-sussman/EDA_Spring2025.git)
