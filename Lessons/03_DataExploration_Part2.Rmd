...
>Answer: Exploring our dataset allows us to find issues that might cause issues later. Ex: When column data types don't accurately relfect what the data actually is: date/time being stored as a factor instead of as data/time or the site id number being stored as a numeric data type instead of as a factor or character.
...
```{r, fig.height = 3, fig.width = 4}
#  Create histogram of the discharge.mean. Histograms are for continuous data (numeric), bar plots are for non-numeric (factors).
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# Bins are now 10 units of(cfs) of discharge (x-axis)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# Change bins to 20 units. aes refers to the data, 
# which is why bins is outside of the aes. bin is a part of the geom
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Bins are set to 10. Focus on values up to 500. Defines the extent of the x-axis.
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# Looking at mean gage height
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# Frequency polygon. mean height of the stream. Is drawing a line that would go over the top of a histogram. Can show more than one distribution/Compare multiple lines. The aes change with each geom.
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgrey") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) +
  scale_x_continuous(limits = c(0, 10))

# In this one, the color is in the aes (last one it was not). It's saying we're going to apply some sort of attribute of our plot based on values in the data frame.
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# boxplot. big flat horizontal line is media=n. box is showing Interquartile range. Everything outside is falling outside of IQR (75th percentile, 25th percentile). The narrower the box the narrower the distribution.
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# When x-axis is continuous. Can be useful for looking at interactive properties of the dataset.
ggplot(USGS.flow.data) +
  geom_boxplot(
    aes(
      x = gage.height.mean, 
...
# the width is propotional to the number of values in there.
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# Every point is a row in the dataframe.
ggplot(USGS.flow.data) +
...
> Answer: bar plot: non-continuous data (like approved/not approved). 
histogram: Visualize distributions of values for continuous numerical variables.
frequency polygon: Can show more than one distribution/Compare multiple lines, like mean/min/max height of discharge. 
Boxplot: want to know Interquartile range ? 
violin: displays density distributions, somewhat like a hybrid of the box-and-whiskers and the frequency polygon plot.
scatterplot: continuous data and how two continuous data variables relate to each other.
