...
huc8_sf_neuse <- huc8_sf %>%
  filter(SUBBASIN == "Upper Neuse")
 

#Create a map
ggplot(data = huc8_sf_neuse) +
   geom_sf(data = huc8_sf, fill = "gray") +
  geom_sf(fill = "yellow", color = "blue")
...
wind_raw <- read.csv(
  ...
  group_by(huc_8) %>%
  summarise(AvgWindEnergyHuc8 = sum(AvgWindEnergy))
  
#Join to HUC_8 features
huc8_wind_join <- merge(x = huc8_sf,
                        y = wind_raw,
                        by.x = "HUC_8",
                        by.y = "huc_8")
...
neuse_intersect <- counties_sf_utm %>%  
  st_filter(neuse_sf, .predicate = st_intersects) # keeps all the counties that touch the watershed

# use predicate = st_disjoint for all the counties that do not intersect the neuse watershed
...
# plot in gg plot
ggplot(triCo_huc8_sf) +
  geom_sf(fill = "purple", alpha = 0.5)

# plot in mapview
mapview(triCo_huc8_sf)
