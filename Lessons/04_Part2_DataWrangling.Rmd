...
NTL.nutrient.data.PeterPaul <- 
  ...
  mutate(month = month(sampledate)) %>% # mutate to create the month column
  select(lakeid:daynum, month, sampledate:comments) %>% # select all the columns to reorder where the month date sits (after day number)
  drop_na(depth) # dropping all the NA's
...
# Spread nutrient data into separate columns using pivot_wider (going back to how it orginally was)
...
# Pipe, read each document in the file
Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
```
