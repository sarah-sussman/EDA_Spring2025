...
library(ggridges) # ridge plots help you look at the distribution of a variable over time

#here() # my here is still not working
...
PeterPaul.chem.nutrients.gathered <-
  read.csv(("./Data/Processed_KEY/NTL-LTER_Lake_Nutrients_PeterPaulGathered_Processed.csv"), stringsAsFactors = TRUE)
...
# Fix this code - fixed. color should be outside if aesthetics.
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + # change maximum value to 250 (right now it is 25,000, which is making the plot small and hard to see).
  ylim(0, 20)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # we want 0 to be on top, and deepest observations to be on lwoer part of y-axis. because talking about depth
print(tempvsdepth)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? fill will change the actual fill of the box which doesn't look good. color changes the outline
print(Nutrientplot3)
