...
# ozone in x axis, pm2.5 in y-aaxis. 
...
# p-value = < 0.05
# R-squared = 0.1316, yes there is aa relaaationship ebvtween ozone anad pm2.5 but ozone is only showing 13% of vaariability of pm2.5. if want better model for pm2.5 need to add more variables
...
PM2.5byAll <- lm(data = EPAair.subset,
                 PM2.5 ~ Ozone + Year + Month +
                   mean_Lng)
step(PM2.5byAll)

#AIC is saying to keep everything there. All are good candiates.
...
# Aall regression cooefficants are <0.05. Meaning there is significant difference.
# Model 19% of PM2.5. still want R^2 to bump up a little bit more.
```
