...
#Update the column headers
colnames(gage_data_exercise) = c(
  "agency_cd", "site_no", "datetime", "tz_cd",
  "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

#Tidy the data
gage_data_exercise <- gage_data_exercise %>% 
  select(datetime, discharge_cfs, gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_exercise,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
the_states <- the_website %>%
  html_nodes('.basic-table a') %>% html_text()
...
df <- data.frame(row.names = c("the_price", "the_capacity", "net_generation", "total_retail"))
```
