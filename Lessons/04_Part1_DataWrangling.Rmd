...
`!=`# want to check if variables are different
`<` # want to check if values are less than
`<=`
`>`
`>=`
`&` # have multiple conditions to check, want both to be true
`|` # if one or the other should be true (Ex. peter lake or paul lake)
...
NTL.phys.data.temp.decending <- arrange(NTL.phys.data, desc(temperature_C))

# East Long Lake and Hummingbird Lake have the highest temperature.
```
### Select
...
# lakeid-daynum, month, sampledate-comments
...
Don't want to do everything in the same documment; you can easily get lost in the middle of the file, makes it longer. Keeps everything neat and organized for yourself and for sharing.
